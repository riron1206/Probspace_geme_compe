...
Usage:
    ...
    $ python train_nn.py -o ../model/nn/code_train/eng2_csv -i ../data/feature_eng/eng2.csv  # LB(submission_kernel.csv): 0.548483, fold_mean_acc: 0.5443158611300205
...
class Encoder:
    ...
    def impute_null_add_flag_col(
        df, strategy="median", cols_with_missing=None, fill_value=None
    ):
        """欠損値を補間して欠損フラグ列を追加する
...
def prepare_data(df, target_col):
    ...
    df = Encoder().impute_null_add_flag_col(
        df, cols_with_missing=null_indexs, strategy="most_frequent"
    )  # 最頻値で補間
...
def enc_df(df_tr, df_va, df_te, target_col, cat_features):
    ...
    df_tr, df_va = Encoder().catboost_encoder(
        df_tr, df_va, target_col=target_col, cat_features=cat_features
    )
    ...
    _df_tr, df_te = Encoder().catboost_encoder(
        _df_tr, df_te, target_col=target_col, cat_features=cat_features
    )
...
def train_nn_cv(
    df_train, df_test, feats, target_col, params, num_folds=2, cat_features=None,
):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(
        folds.split(df_train[feats], df_train[target_col])
    ):
        print("n_fold:", n_fold)

        # target_encoderとか
        df_tr, df_va, df_te = enc_df(
            df_train.loc[train_idx],
            df_train.loc[valid_idx],
            df_test,
            target_col,
            cat_features,
        )
...
if __name__ == "__main__":
    ...
    df_train, df_test, cat_features = prepare_data(df_all, target_col)
    ...
    # df_train, df_test, cat_features = prepare_data(df, target_col)
    # display(df_test)
